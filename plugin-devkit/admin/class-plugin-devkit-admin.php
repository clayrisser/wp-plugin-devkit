...
class Plugin_Devkit_Admin {
	...
	/**
	 * The hook suffix of the plugin options page.
	 *
	 * @since    0.0.1
	 * @access   private
	 * @var      string|false    $options_page    Hook suffix of the options page, or false if not registered.
	 */
	...
	public function register_settings() {
		...
		register_setting(
			...
			array(
				'type'              => 'string',
				'sanitize_callback' => array( $this, $this->option_name . '_sanitize_fav_text_editor' ),
			)
		);
	}
	...
	public function plugin_devkit_general_cb() {
		echo '<p>' . esc_html__( 'Please change the settings accordingly.', 'plugin-devkit' ) . '</p>';
	}
	...
	public function plugin_devkit_fav_text_editor_cb() {
		...
		?>
			<fieldset>
				<label>
					<input type="radio" id="<?php echo esc_attr( $option_name ); ?>" name="<?php echo esc_attr( $option_name ); ?>" value="vim" <?php checked( $fav_text_editor, 'vim' ); ?>>
					<?php esc_html_e( 'Vim', 'plugin-devkit' ); ?>
				</label>
				<br />
				<label>
					<input type="radio" name="<?php echo esc_attr( $option_name ); ?>" value="emacs" <?php checked( $fav_text_editor, 'emacs' ); ?>>
					<?php esc_html_e( 'Emacs', 'plugin-devkit' ); ?>
				</label>
			</fieldset>
		<?php
	}
